...
- **代码编辑 IDE 层**：默认 profile 启用 `vim-lsp`、`vim-lsp-settings`、`asyncomplete`、`ALE`、`editorconfig-vim`、`vim-test`，覆盖跳转、诊断、补全、格式化与测试入口。
...
3. 安装过程中脚本会检测 **`vim`、`rg`、`fzf`、`node`、`npm`**，并在默认 profile 中额外检查 `shellcheck`、`shfmt` 与 npm 语言工具；缺失时请先执行 `./vim-manager deps`，或使用下方「全自动」脚本。
...
| 命令 | 说明 |
|------|------|
| `./vim-manager deps` | 安装外部依赖（如 `vim`、`rg`、`fzf`、`node`、`shellcheck`、`shfmt`、npm 语言工具等） |
...
`install` / `update` / `status` / `clean` 可附加 `--profile minimal|default|full`。不传参数时使用 `default`；`minimal` 只处理基础编辑与检索插件，不要求 LSP、补全、诊断和测试插件；`full` 当前等同 `default`。`clean` 始终以完整托管清单保护已安装插件，避免 profile 切换造成误删。
...
vim-manager
vim_manager_modules/
├── config.sh      # 版本、插件清单、REQUIRED_TOOLS、IDE_TOOLS
├── helpers.sh
...
与 `vim_manager_modules/config.sh` 中 **`PLUGINS`** 保持一致，当前包含但不限于：`vim-airline`、`nerdtree`、`vim-devicons`、`vim-fugitive`、`vim-surround`、`vim-repeat`、`vim-commentary`、`vim-signify`、`vim-polyglot`、`auto-pairs`、`vim-sensible`、`undotree`、`vim-easy-align`、**`fzf.vim`**、**`csv.vim`**、`markdown-preview.nvim`、`vim-which-key`、`vim-lsp`、`vim-lsp-settings`、`asyncomplete.vim`、`asyncomplete-lsp.vim`、`ale`、`editorconfig-vim`、`vim-test`。具体以清单为准。
...
- 如需关闭默认 IDE 层，可在 `~/.vimrc.before.local` 中设置 `let g:vim_settings_no_ide = 1`。
...
1. `./vim-manager status` 逐项查看缺失工具与插件；默认 profile 会额外列出 IDE 工具状态。
